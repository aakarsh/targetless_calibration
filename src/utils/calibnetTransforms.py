...
def _simple_transformer(depth_map, t_mat, k_mat, small_transform):
    ...
    x_all = torch.reshape(batch_grids[:,0], (IMG_HT, IMG_WDT))
    y_all = torch.reshape(batch_grids[:,1], (IMG_HT, IMG_WDT))
...
def sparsify_cloud(S):
    ...
    with torch.device('cpu:0'):
        ...
        saved_points = tf.gather_nd(S, [torch.unsqueeze(torch.range(0, no_partitions*point_limit), 1)])
        saved_points = tf.reshape(saved_points, [point_limit, no_partitions, 3])
        saved_points_sparse = tf.reduce_mean(saved_points, 1)
        '''
        no_partitions = no_points/torch.constant(point_limit, dtype=tf.int32)
        saved_points = torch.index_select(S, [torch.unsqueeze(torch.range(0, no_partitions*point_limit), dims=1)])
        saved_points = torch.reshape(saved_points, [point_limit, no_partitions, 3])
...
def _3D_meshgrid_batchwise_diff(height, width, depth_img, num_batch, transformation_matrix, tf_K_mat, small_transform):
    ...
    '''
    sampling_grid_2d = torch.cat([x_t_flat, y_t_flat, ones], 0)
    sampling_grid_2d_sparse = torch.transpose(torch.masked_select(torch.transpose(sampling_grid_2d), mask))
    ...
    '''
    projection_grid_3d = torch.matmul(torch.inverse(tf_K_mat), sampling_grid_2d_sparse*ZZ_saved)

    '''
    homog_points_3d = tf.concat([projection_grid_3d, ones_saved], 0)
    '''
    homog_points_3d = torch.cat([projection_grid_3d, ones_saved], 0)
    ...
    '''
    final_transformation_matrix = torch.matmul(transformation_matrix, small_transform)[:3,:]
    warped_sampling_grid = torch.matmul(final_transformation_matrix, homog_points_3d)

    '''
    points_2d = tf.matmul(tf_K_mat, warped_sampling_grid[:3,:])
    '''
    points_2d = torch.matmul(tf_K_mat, warped_sampling_grid[:3,:])
    ...
    '''
    point_cloud = torch.stack([x_dash_pred, y_dash_pred, Z], dims=1)
    ...
    '''
    x = torch.transpose(points_2d[0,:]/Z)
    y = torch.transpose(points_2d[1,:]/Z)

    '''
    mask_int = tf.cast(mask, 'int32')
    '''
    mask_int = mask.type(torch.IntTensor)

    '''
    updated_indices = tf.expand_dims(tf.boolean_mask(mask_int*z_index, mask), 1)
    '''
    updated_indices = torch.unsqueeze(tf.masked_select(mask_int*z_index, mask), 1)
    ...
    '''
    updated_Z = torch.index_add(updated_indices, Z, torch.constant([width*height]))
    updated_x = torch.index_add(updated_indices, x, torch.constant([width*height]))
    ...
    '''
    updated_y =  torch.index_add(updated_indices, y, tf.constant([width*height]))
    updated_y_fin = torch.where(torch.equal(updated_Z, zeros_target), neg_ones, updated_y)
    ...
    '''
    transformed_depth = torch.reshape(updated_Z, (IMG_HT, IMG_WDT))
...
def reverse_all(z):
    ...
    t = (w**2 + w)/2.0
    y = torch.clamp(torch.unsqueeze(z - t, 1), 0.0, IMG_HT - 1)
    x = torch.clamp(torch.unsqueeze(w - y[:,0], 1), 0.0, IMG_WDT - 1)
...
def get_pixel_value(img, x, y):
    ...
    with torch.device('cpu:0'):
        ...
        indices = torch.reshape(indices, (375*1242, 2))
        ...
        filtered, idx = torch.unique(torch.squeeze(Z))
        '''
        updated_values  = tf.unsorted_segment_max(values, idx, torch.Size(filtered)[0])
        '''
        updated_values  = tf.unsorted_segment_max(values, idx, torch.Size(filtered)[0])

        # updated_indices = tf.map_fn(fn=lambda i: reverse(i), elems=filtered, dtype=tf.float32)
        updated_indices = reverse_all(filtered)
        updated_indices = updated_indices.type(torch.IntTensor)

        resolved_map = torch.index_add(updated_indices, updated_values, img.shape)
...
def _bilinear_sampling(img, x_func, y_func):
    ...
    '''
    max_y = torch.constant(IMG_HT - 1, dtype=tf.int32)
    max_x = torch.constant(IMG_WDT - 1, dtype=tf.int32)
    ...
    '''
    x = 0.5 * ((x_func + 1.0) * torch.tensor(IMG_WDT - 1).type(torch.FloatTensor))
    y = 0.5 * ((y_func + 1.0) * torch.tensor(IMG_WDT - 1).type(torch.FloatTensor))

    x = torch.clamp(x, 0.0, max_x.type(torch.FloatTensor))
    y = torch.clamp(y, 0.0, max_y.type(torch.FloatTensor))
    ...
    x0 = torch.floor(x).type(torch.IntTensor)
    x1 = x0 + 1
    y0 =  torch.floor(y).type(torch.IntTensor)
    y1 = y0 + 1
